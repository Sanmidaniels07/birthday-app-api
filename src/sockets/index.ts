...
export function getIO(): Server {
  if (!io) throw new Error('Socket.IO not initialized — initSockets(server) must run at boot');
  return io;
}
...
async function broadcastPresence(userId: string, online: boolean): Promise<void> {
  ...
  for (const friendId of friendIds) {
    getIO().to(`user:${friendId}`).emit(SocketEvents.PRESENCE_CHANGED, { userId, online });
  }
}
...
export function initSockets(server: HttpServer): Server {
  ...
  io.on('connection', async (socket) => {
    ...
    // ---- Presence in ----
    const firstSocket = !presence.has(userId) || presence.get(userId)!.size === 0;
    ...
    await Promise.all(memberships.map((m) => socket.join(`conversation:${m.conversationId}`)));

    logger.debug({ userId, socketId: socket.id }, 'socket connected');

    // ---- Typing: pure relay, membership-checked, never persisted ----
    socket.on(SocketEvents.CLIENT_TYPING_START, async (payload: { conversationId?: string }) => {
      ...
      socket.to(`conversation:${conversationId}`).emit(SocketEvents.TYPING_STARTED, {
        conversationId,
        userId,
      });
    });

    socket.on(SocketEvents.CLIENT_TYPING_STOP, (payload: { conversationId?: string }) => {
      const conversationId = payload?.conversationId;
      if (!conversationId) return;
      if (!socket.rooms.has(`conversation:${conversationId}`)) return;
      socket.to(`conversation:${conversationId}`).emit(SocketEvents.TYPING_STOPPED, {
        conversationId,
        userId,
      });
    });
    ...
    socket.on('disconnect', async () => {
      ...
      if (sockets && sockets.size === 0) {
        ...
        await prisma.user
          ...
          .catch((err) => logger.error({ err, userId }, 'lastSeenAt update failed'));
      }
